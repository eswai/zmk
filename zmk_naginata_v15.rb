...
def teigi(a, b, c, prefix="", suffix="")
  ...
  ok = b.upcase.scan(/./) + ['NONE', 'NONE', 'NONE', 'NONE', 'NONE', 'NONE']
  ok = ok[0..5].join(", ")
  sprintf("  {%-32s", ".key = #{prefix}#{_a}#{suffix}") + sprintf("%-20s", ", .kana = {#{ok}}") + "}, // #{c}"
end
...
daku.each_with_index do |k, i|
  ...
  if j && j >= 0
    if eiji_r.index(eiji[j])
      ...
      # puts teigi(eiji[j], r_daku[i], k + "(冗長)", "B_F|", "|B_SPACE")
    else
      puts teigi(eiji[j], r_daku[i], k, "B_J|")
      # puts teigi(eiji[j], r_daku[i], k + "(冗長)", "B_J|", "|B_SPACE")
    end
  end
end
...
handaku.each_with_index do |k, i|
  ...
  if j && j >= 0
    if eiji_r.index(eiji[j])
      ...
      # puts teigi(eiji[j], r_handaku[i], k + "(冗長)", "B_V|", "|B_SPACE")
    else
      puts teigi(eiji[j], r_handaku[i], k, "B_M|")
      # puts teigi(eiji[j], r_handaku[i], k + "(冗長)", "B_M|", "|B_SPACE")
    end
  end
end
...
kogaki.each_with_index do |k, i|
  ...
  if j && j >= 0
    ...
    # puts teigi(eiji[j], r_kogaki[i], k, "B_Q|B_SPACE|")
    # puts teigi(eiji[j], r_kogaki[i], k, "B_V|B_M|")
    # puts teigi(eiji[j], r_kogaki[i], k + "(冗長)", "B_V|B_M|", "|B_SPACE")
  end
end
